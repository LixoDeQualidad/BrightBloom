...
@export var victory_scene: PackedScene # arraste aqui a cena de vitória no Inspector
@export var time_before_victory: float = 1.5
...
@onready var dialogue_box = get_node("/root/Sala3/Npc4/DialogueBox")
...
func _get_current_lines() -> Array[String]:
	...
	elif GameManager.collected_count >= required_items:
...
func _check_reward():
	if not quest_given and GameManager.collected_count >= required_items:
		quest_given = true
...
func _go_to_victory() -> void:
	if victory_scene:
		await get_tree().create_timer(time_before_victory).timeout
